...
FluentSender.prototype._doWrite = function (packet, options, timeoutId, callbacks) {
  if (!this._socket) {
      this._sendQueue = [];
      this.internalLogger.info('Queue flushed due to an error in _doWrite');
      let error = FluentLoggerError.ConnectionError('socket is missing');
...
FluentSender.prototype._setupErrorHandler = function _setupErrorHandler() {
  ...
  this.on('error', (error) => {
    ...
    this.internalLogger.info('Queue flushed due to an error');
